...
// Map of sequenceId -> name via the Sequences API (scope automation.sequences.read).
// Returns {} on failure so the dashboard still works (falls back to "Sequence <id>").
export async function fetchSequenceNames() {
  const map = {};
  let after = undefined;
  do {
    const q = new URLSearchParams({ limit: "100" });
    if (after) q.set("after", after);
    const data = await hub(`/automation/sequences/2026-03?${q.toString()}`);
    for (const s of data.results || []) {
      if (s && s.id != null) map[String(s.id)] = s.name || `Sequence ${s.id}`;
    }
    after = data.paging?.next?.after;
  } while (after);
  return map;
...
// All contacts whose LAST sequence enrollment date falls in [start,end], across
// ALL sequences. This is HubSpot's "enrolled" universe (matches Total Enrollments).
export async function fetchEnrolledContacts(start, end, props) {
  const out = [];
  let after = undefined;
  do {
    const body = {
      filterGroups: [{
        filters: [
          { propertyName: "hs_latest_sequence_enrolled_date", operator: "BETWEEN", value: String(toMs(start)), highValue: String(toMs(end, true)) },
          { propertyName: "hs_latest_sequence_enrolled", operator: "HAS_PROPERTY" },
        ],
      }],
      properties: props,
      limit: 100,
    ...
    for (const r of data.results || []) out.push({ id: r.id, ...(r.properties || {}) });
    after = data.paging?.next?.after;
  } while (after);
  return out;
}
